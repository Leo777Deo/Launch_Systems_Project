...
M1.fairing = 40; %[kg] fairing of the first stage is nonexistent
...
M2.fairing = 40; %[kg] fairing of the second stage (31.8 / 31.9)
...
f4 = @(k) dv - c(1) * log( (c(1)-k)/(c(1)*e(1)) ) - c(2) * log( (c(2)-k)/(c(2)*e(2)) );
k = -1:1e-3:15;
lim = min( c.*(1-e));
lim = min( c -eps);
figure(2);
plot(k, f4(k)); hold on; grid on;

fzero(f4, [0, lim])

f5 = @(k) dv - c' * log( (c-k) ./ (c.*e) );
plot(k, f5(k), '--k');
plot([min(c), min(c)], [-5, 5]);
K = fzero(f5, [0, lim]);
